const fields = [
  ...
  ['utilizationHours','Utilisation',8,'hours/day'],
  ...
  ['productivityMultiplier','API/cloud efficiency multiplier',1.15,'x']
];
const presets = {
...
function riskNotes(r){
  const notes = [];
  if (num('utilizationHours') < 6) notes.push('Low utilisation makes local hardware harder to justify. Rent or API-first may preserve cash.');
  if (num('maintenanceHours') > 6) notes.push('Maintenance assumptions are high. Standardise deployment and monitoring before buying more hardware.');
...
function render(){
  ...
  recommendation.innerHTML = `<h2>${r.title}</h2><p>${r.delta >= 0 ? 'Local saves against the modelled API/cloud baseline.' : 'API/cloud is cheaper under current assumptions.'}</p><p><span class="status">${r.mode.toUpperCase()}</span></p>`;
  breakdown.innerHTML = [
